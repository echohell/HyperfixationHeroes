...
var skills_list = [["attack_melee"], ["attack_ranged"], ["attack_magic"]]  # array of basic attacks
...
func attack_melee(attacker: Dictionary, target: Dictionary):                # resolves melee attack
	attack(attacker, target, "attack_melee")

func attack_ranged(attacker: Dictionary, target: Dictionary):              # resolves ranged attack
	attack(attacker, target, "attack_ranged")

func attack_magic(attacker: Dictionary, target: Dictionary):                # resolves magic attack
	attack(attacker, target, "attack_magic")
	var skill = SkillData.skills["attack_magic"]
	do_damage(attacker, target, skill)                                  # don't need to check range
...
func attack(attacker: Dictionary, target: Dictionary, _attack: String):
	...
	if valid:
		...
		if random_number < prob:                           # if the random number beats probability
			...
			update_information.emit("Missed. \n\n")              # update if the probability missed
...
func ai_process(comb : Dictionary):
	...
	if comb.class == UnitClass.Melee:                                   # if the combatant is melee
		...
		if get_distance(comb, nearest_target) == 1:   # get the distance to check if in melee range
			attack(comb, nearest_target, "attack_melee")                        # attack with melee
			return                                                                    # then return
			
	await controller.ai_process(nearest_target.position)                # else wait for ai progress
	attack(comb, nearest_target, "attack_melee")                                       # and attack
